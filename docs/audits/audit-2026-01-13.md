# Auditoría Completa RAG Corp (HISTORICAL)
...
|------------|------------|-----------|-----------------|
| **Backend** | FastAPI + Python 3.11 | `apps/backend/` | API REST, orquestación RAG |
| **Frontend** | Next.js 16 + TypeScript | `apps/frontend/` | UI de chat y búsqueda |
...
│                         FLUJO DE INGESTA                            │
├─────────────────────────────────────────────────────────────────────┤
│  Cliente → POST /v1/ingest/text → IngestDocumentUseCase             │
│     1. Validación (Pydantic)                                        │
│     2. Chunking (SimpleTextChunker: 900 chars, 120 overlap)         │
│     3. Embedding batch (GoogleEmbeddingService → 768D vectors)      │
│     4. Persist atómico (PostgresDocumentRepository)                 │
│  Respuesta: { document_id, chunks_created }                         │
└─────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────┐
│                         FLUJO DE CONSULTA                           │
├─────────────────────────────────────────────────────────────────────┤
│  Usuario → POST /v1/ask → AnswerQueryUseCase                        │
│     1. Embed query (GoogleEmbeddingService, task_type=retrieval)    │
│     2. Retrieval (pgvector cosine similarity, top-k chunks)         │
│     3. Context building (ContextBuilder + metadata grounding)       │
│     4. LLM generation (Gemini 1.5 Flash + prompt versionado)        │
...
```
┌───────────────────────────────────────┐
│  API Layer (main.py, routes.py)       │  ← HTTP, validación, serialización
├───────────────────────────────────────┤
│  Application (use_cases/)             │  ← Orquestación, lógica de negocio
├───────────────────────────────────────┤
│  Domain (entities.py, protocols)      │  ← Entidades, interfaces (puros)
├───────────────────────────────────────┤
│  Infrastructure (services/, repos/)   │  ← Implementaciones concretas
└───────────────────────────────────────┘
```
...
| `app/config.py` | Settings tipados | Centraliza env vars con validación |
...
| `app/auth.py` | API Key auth | Validación de keys + scopes |
...
| `app/metrics.py` | Prometheus | Métricas de latencia y conteo |
...
| `tests/integration/` | Tests integración | Requieren DB/API |
...
| Carpeta | Rol | Por qué existe |
|---------|-----|----------------|
| `postgres/init.sql` | Schema DDL | Crea tablas + índice IVFFlat |
| `k8s/` | K8s manifests | Deployment, HPA, Ingress, PDB |
| `grafana/` | Dashboards | Visualización métricas |
| `prometheus/` | Scrape config | Configuración Prometheus |
...
| `data/postgres-schema.md` | Schema docs | Tablas e índices |
...
| `compose.prod.yaml` | Compose producción |
...
- **API layer** solo maneja HTTP/serialización

**Evidencia:**
- [entities.py](apps/backend/app/domain/entities.py): Sin imports de FastAPI/psycopg
- [answer_query.py](apps/backend/app/application/use_cases/answer_query.py): Usa Protocol, no impl concreta
...
- Structured error responses con `error_id` para correlación
- Exception handlers centralizados en [exception_handlers.py](apps/backend/app/exception_handlers.py)
- Never log raw API keys (hash en logs)

**Evidencia:**
- [exceptions.py](apps/backend/app/exceptions.py): Hierarchy clara
- HTTP 503 para service errors, 500 para generic

### 3.4 Tipado/Contratos ✅
...
- Cliente TS generado con Orval → [generated.ts](shared/contracts/src/generated.ts)

### 3.5 Configuración ✅

**Estado: MUY BUENO**

- Centralizada en [config.py](apps/backend/app/config.py) con `pydantic-settings`
- Validación de env vars al startup (fail-fast)
- Defaults sensatos documentados
- [.env.example](.env.example) completo (139 líneas)

### 3.6 Logging/Observabilidad ✅

**Estado: MUY BUENO**

- JSON structured logging en [logger.py](apps/backend/app/logger.py)
- Request context propagation (`request_id`, `trace_id`)
- Prometheus metrics en [metrics.py](apps/backend/app/metrics.py)
- Stage timings (`embed_ms`, `retrieve_ms`, `llm_ms`)
- Health check en `/healthz` con verificación de DB y Google API

### 3.7 Seguridad ✅

**Estado: MUY BUENO**

- API Key auth con scopes ([auth.py](apps/backend/app/auth.py))
- RBAC con roles jerárquicos ([rbac.py](apps/backend/app/rbac.py))
- Constant-time comparison (previene timing attacks)
- Security headers OWASP ([security.py](apps/backend/app/security.py))
- Rate limiting token bucket ([rate_limit.py](apps/backend/app/rate_limit.py))
- Body size limit middleware
- CORS configurable

**Mejora:** RBAC_CONFIG aún no se usa en endpoints (solo definido)
...
- Fixtures bien diseñadas en [conftest.py](apps/backend/tests/conftest.py)
...
| 2 | **RBAC definido pero no integrado** | Feature incompleta, seguridad parcial | [rbac.py](apps/backend/app/rbac.py) define roles pero `routes.py` usa solo `require_scope` |
| 3 | **Rate limit in-memory** | Se pierde al restart, no escala horizontalmente | [rate_limit.py#L27](apps/backend/app/rate_limit.py#L27): "_In-memory storage_" |
| 4 | **Tests de frontend mínimos** | Riesgo de bugs en UI | Solo 2 archivos en `apps/frontend/__tests__/` |

### 🟡 MEDIA PRIORIDAD

| # | Issue | Impacto | Evidencia |
|---|-------|---------|-----------|
| 5 | **Sin soft-delete real** | `deleted_at` en entity pero no usado | [entities.py#L49](apps/backend/app/domain/entities.py#L49): campo definido, sin impl |
| 6 | **Chunking solo simple** | Chunks pueden cortar mid-sentence | [SimpleTextChunker](apps/backend/app/infrastructure/text/) no usa semantic boundaries |
| 7 | **No hay cache de embeddings en prod** | API calls repetidas innecesarias | `REDIS_URL` opcional, no obligatorio |
| 8 | **Pool DB no tiene retry** | Falla en transient errors de conexión | [postgres_document_repo.py](apps/backend/app/infrastructure/repositories/postgres_document_repo.py) no retry pool errors |

### 🟢 BAJA PRIORIDAD

| # | Issue | Impacto | Evidencia |
|---|-------|---------|-----------|
| 9 | **Load tests vacíos** | No hay baseline de performance | `tests/load/` existe pero vacío |
| 10 | **Docs desactualizadas** | Onboarding más lento | Algunos archivos en `_legacy_candidates/` |

---

## 5. Quick Wins (1-2 horas)

### 5.1 Agregar GitHub Actions básico
**Tiempo:** 1 hora  
**Impacto:** Validación automática en PRs
...
jobs:
  backend:
    ...
    steps:
      ...
      - run: cd apps/backend && pytest -m unit --cov=app
```
...
Agregar tabla con variables críticas y sus valores por defecto.

---

## 6. Mejoras Medianas (1-2 días)

### 6.1 Integrar RBAC en endpoints
**Tiempo:** 1 día  
**Archivo:** [routes.py](apps/backend/app/routes.py)
...
**Tiempo:** 1 día  
**Archivos:** [rate_limit.py](apps/backend/app/rate_limit.py), [compose.yaml](compose.yaml)
...
**Tiempo:** 1-2 días  
**Archivo:** [SimpleTextChunker](apps/backend/app/infrastructure/text/)
...
- Agregar 5-10 tests críticos: login, ingest, ask
- Integrar en CI
...
Extraído de [README.md](README.md) y [runbook/local-dev.md](docs/runbook/local-dev.md):
...
# 6. Generar contratos (opcional, si cambió API)
...
# → {"ok":true,"db":"connected","request_id":"..."}
...
3. **Observabilidad** - Logs estructurados, métricas Prometheus
4. **Contratos tipados** - OpenAPI → TypeScript
5. **Documentación** - README, runbooks, architecture docs

### Áreas de Mejora 🔧
1. **CI/CD** - No hay automatización
...
El repo está en **buen estado** para un proyecto en desarrollo activo. Priorizar:
1. CI/CD básico (quick win con alto impacto)
2. Aumentar coverage de tests
3. Integrar RBAC en endpoints

---

*Auditoría generada automáticamente. Verificar manualmente cualquier afirmación crítica.*
