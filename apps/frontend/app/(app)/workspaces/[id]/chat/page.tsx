                ...
                >
                  ...
                  >
                    ...
                    {statusText && message.content ? (
                      <p className={`mt-2 text-xs ${isUser ? "text-cyan-200" : "text-white/40"}`}>
                        {statusText}
                      </p>
                    ) : null}
                    {message.verifiedSources && message.verifiedSources.length > 0 ? (
                      <div className={`mt-4 border-t pt-3 ${isUser ? "border-cyan-400/30" : "border-white/10"}`}>
                        <p className={`text-xs uppercase tracking-[0.2em] ${isUser ? "text-cyan-200" : "text-white/40 font-bold"}`}>
                          Fuentes verificadas
                        </p>
                        <ul className={`mt-2 space-y-2 text-xs ${isUser ? "text-cyan-100" : "text-white/70"}`}>
                          {message.verifiedSources.map((source) => (
                            <li
                              key={source.chunk_id}
                              className={`rounded-xl border p-3 ${isUser
                            ...
                            >
                              <div className="flex flex-wrap items-center justify-between gap-2">
                                <div>
                                  <p className={`text-xs ${isUser ? "text-cyan-300" : "text-white/40"}`}>
                                    Documento
                                  </p>
                                ...
                                <Link
                                  ...
                                  className={`rounded-full border px-3 py-1 text-[10px] uppercase tracking-[0.2em] transition ${isUser
                              ...
                              <p className={`mt-2 text-[11px] font-mono ${isUser ? "text-cyan-300" : "text-white/40"}`}>
                      ...
                      <div className={`mt-4 border-t pt-3 ${isUser ? "border-cyan-400/30" : "border-white/10"}`}>
                        ...
                        <ul className={`mt-2 space-y-2 text-xs ${isUser ? "text-cyan-100" : "text-white/70"}`}>
                          {message.sources.map((source) => (
                            <li
                              key={source.chunk_id}
                              className={`rounded-xl border p-2 ${isUser
                                  ? "border-cyan-400/30 bg-cyan-800/20"
                                  : "border-white/10 bg-black/20"
                                }`}
