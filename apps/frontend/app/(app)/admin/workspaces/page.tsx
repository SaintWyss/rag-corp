...
export default function AdminWorkspacesPage() {
  ...
  // Load workspaces when selected user changes
  useEffect(() => {
      ...
      <section className="space-y-8">
        <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
          <div className="flex flex-col gap-2">
            ...
            <h1 className="text-3xl font-semibold text-white">Workspaces Provisioning</h1>
            <p className="text-sm text-white/60">
              Crea workspaces directamente para cualquier usuario de la organización.
            </p>
          </div>
        </div>
        ...
        {authChecked && !isAdmin ? (
          <div className="rounded-3xl border border-rose-400/30 bg-rose-500/10 p-6 text-rose-100">
            Acceso restringido. Necesitas un usuario admin para aprovisionar workspaces.
          </div>
        ) : (
          <div className="grid gap-8 lg:grid-cols-[1fr,2fr]">
            ...
            <div className="space-y-6">
              {/* User Selection */}
              <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
                <h2 className="mb-4 text-lg font-bold text-white">1. Seleccionar Usuario</h2>
                {usersLoading ? (
              ...
              <div className="rounded-3xl border border-white/10 bg-white/5 p-6 opacity-100 transition-opacity">
                <h2 className="mb-4 text-lg font-bold text-white">2. Crear Workspace</h2>
                <form onSubmit={handleCreate} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      disabled={!selectedUserId}
                      placeholder="Workspace para campañas..."
                      className="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-2 text-sm text-white placeholder:text-white/30 focus:border-rose-400 focus:outline-none focus:ring-2 focus:ring-rose-400/20 disabled:opacity-50"
                    />
                  </div>
            ...
            {/* Right Column: User's Workspaces List */}
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6 min-h-[400px]">
              <div className="mb-6 flex items-center justify-between">
                <h2 className="text-lg font-bold text-white">Workspaces del Usuario</h2>
                {selectedUserId && (
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <p className="text-xs text-white/40 font-mono mt-1">{ws.id}</p>
                        </div>
                        <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider ${
                          ws.visibility === "PRIVATE" ? "bg-white/5 text-white/50" : 
                          ws.visibility === "ORG_READ" ? "bg-cyan-500/10 text-cyan-200" : 
                      ...
                      <div className="mt-3 flex gap-4 text-xs text-white/40">
                         ...
                         {ws.archived_at && <span className="text-rose-300">Archivado</span>}
                      </div>
                    </div>
                  ))}
                </div>
