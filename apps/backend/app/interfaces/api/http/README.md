...
- No contiene lógica de negocio ni acceso a DB. Razón: ** negocio y orquestación pertenecen a Application/Domain; persistencia a Infrastructure. Impacto: ** si un endpoint necesita “algo de DB”, eso se hace vía un caso de uso y repositorios; no desde el router.

- No ejecuta tareas de background. Razón: ** procesamiento asíncrono va por worker/cola. Impacto: ** el endpoint encola un job y devuelve un estado/ID; el worker hace el trabajo.

- No decide implementaciones (Google vs fake, Postgres vs in-memory). Razón: ** esa decisión es del container. Impacto: ** los routers piden use cases al container; no instancian infraestructura.
...
- Crear/ajustar schemas en `schemas/`.
- Crear router en `routers/` e incluirlo en `router.py`.
- Mapear errores nuevos en `error_mapping.py`.
- Cablear dependencias en `app/container.py`.
- Tests: unit de schemas en `apps/backend/tests/unit/api/`, integration de endpoints en `apps/backend/tests/integration/`.

## 🆘 Troubleshooting
- **Síntoma:** 422 inesperado.
- **Causa probable:** schema desalineado.
- **Dónde mirar:** `schemas/`.
- **Solución:** ajustar validación.
- **Síntoma:** error sin RFC7807.
- **Causa probable:** mapping faltante.
- **Dónde mirar:** `error_mapping.py`.
- **Solución:** mapear error tipado.
- **Síntoma:** endpoint no aparece en OpenAPI.
- **Causa probable:** router no incluido en `router.py`.
- **Dónde mirar:** `router.py`.
- **Solución:** incluir router.
- **Síntoma:** streaming corta.
- **Causa probable:** excepción durante stream.
- **Dónde mirar:** `routers/query.py`.
